...
then
    # create mhs files
    python mhsaddhwts.py "-reconf" $RECONOS_ARCH system.mhs.orig $num_static_hwts $num_reconf_regions $static_hwts "reconos_hwt_idle_v1_00_a" > system_static.mhs
    ln -s system_static.mhs system.mhs

    for hwt in $reconf_hwts
    do
        python mhsaddhwts.py "-reconf" $RECONOS_ARCH system.mhs.orig $num_static_hwts $num_reconf_regions $static_hwts $hwt > system_reconf_$hwt.mhs
    done
    ...
    if $RECONOS_MMU
    then
        python mhsaddhwts.py $RECONOS_ARCH system.mhs.orig $num_static_hwts 0 $static_hwts > system.mhs
    else
        python mhsaddhwts.py "-nommu" $RECONOS_ARCH system.mhs.orig $num_static_hwts 0 $static_hwts > system.mhs
    fi
fi