...
(function() {
  ...
  __createBrush = function(component) {
    ...
    svg.append("path").datum(draw_data).attr("class", "line").attr("d", line);
    context = svg.append("g").attr("class", "context").attr("width", width).attr("height", height);
    svg.append("g").attr("class", "x brush").call(brush).selectAll("rect").attr("y", -6).attr("height", height);
  };
  ...
  __updateGlobalCharts = function() {
    var c, chart, _i, _len, _results;
    _results = [];
    for (_i = 0, _len = global_component_charts.length; _i < _len; _i++) {
      c = global_component_charts[_i];
      chart = c.chart;
      _results.push(chart.update());
    }
    return _results;
  };
